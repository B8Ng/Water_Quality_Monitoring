...
TIM_HandleTypeDef htim8;	// TIM_HandleTypeDef 结构体变量
...
void  LCD_PwmSetPulse (uint8_t pulse)
{
	...
	TIM8->CCR4 = compareValue; 			// 修改TIM8的通道4比较值
}
...
void  LCD_PWMinit(uint8_t pulse)
{
	...
	htim8.Instance 					= LTDC_PWM_TIM;							// 定时器
	htim8.Init.Prescaler 			= 23;                              // 预分频系数，此时定时器的计数频率为 10 MHz
	htim8.Init.CounterMode 			= TIM_COUNTERMODE_UP;               // 向上计数模式
	htim8.Init.Period 				= LCD_PwmPeriod -1 ;                // 重载值499，即计数500次
	htim8.Init.ClockDivision 		= TIM_CLOCKDIVISION_DIV1;           // 时钟不分频
	htim8.Init.AutoReloadPreload 	= TIM_AUTORELOAD_PRELOAD_DISABLE;   // 控制寄存器 TIMx_CR1 的ARPE 位置0，即禁止自动重载寄存器进行预装载

	HAL_TIM_Base_Init(&htim8) ;	// 根据上面的参数，对TIM进行初始化

	sClockSourceConfig.ClockSource = TIM_CLOCKSOURCE_INTERNAL;			// 选择内部时钟源
	HAL_TIM_ConfigClockSource(&htim8, &sClockSourceConfig);           // 初始化配置时钟源

	HAL_TIM_PWM_Init(&htim8) ;		// 根据上面的参数，对TIM进行PWM初始化   
	...
	HAL_TIMEx_MasterConfigSynchronization(&htim8, &sMasterConfig);    // 初始化配置
	...
	HAL_TIM_PWM_ConfigChannel(&htim8, &sConfigOC, LTDC_PWM_TIM_CHANNEL);		// 初始化配置PWM

	HAL_TIM_MspPostInit(&htim8);								// 初始化IO口
	HAL_TIM_PWM_Start(&htim8,LTDC_PWM_TIM_CHANNEL);		// 启动PWM			
}
...
/*************************************************************************************************
*	函 数 名:	LCD_PwmGetPulse
*	入口参数:	无
*	返 回 值:	uint8_t - 当前占空比0-100%
*	函数功能:	获取当前PWM占空比
*	说    明:	基于TIM8通道4的CCR寄存器值和预定义周期计算
*************************************************************************************************/
